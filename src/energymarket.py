...
# Add new WholesaleMarketEnv class
class WholesaleMarketEnv(Env):
    ...
    def __init__(
        ...
        self.current_wholesale_price = self.wholesale_prices[0] if len(self.wholesale_prices) > 0 else 50.0
        ...
        self.observation_space = Dict(
            {
                ...
                "market_stats": Box(
                    ...
                    high=np.array([MAX_PRICE, 1000.0], dtype=np.float32),  # [price, total_traded]
                    shape=(2,),
                    dtype=np.float32,
                ),
        "price_forecast": Box( 
            low=np.array([-100.0] * 24, dtype=np.float32),
            high=np.array([500.0] * 24, dtype=np.float32), 
            shape=(24,),
            dtype=np.float32,
        ),
    })
        ...
        print(f"Price range: €{min(self.wholesale_prices):.2f} - €{max(self.wholesale_prices):.2f}")
    ...
    def _get_wholesale_price_forecast(self) -> list[float]:
        ...
        while len(prices) < 24:
            prices.append(self.wholesale_prices[-1] if len(self.wholesale_prices) > 0 else 50.0)
        
        return prices[:24]  # Ensure exactly 24 prices
    ...
    def _execute_wholesale_trading(self) -> list[dict]:
        ...
        for agent in self.agents:
            ...
            if net_demand > 0:
                ...
                trade_value = -net_demand * self.current_wholesale_price  # Negative (cost)
                action_type = 'buy'
            elif net_demand < 0:
                # Agent has surplus energy (selling)
                trade_value = -net_demand * self.current_wholesale_price  # Positive (revenue)
                action_type = 'sell'
            else:
                # No trading needed
                trade_value = 0.0
    ...
    def reset(
        ...
        # Reset agents
        for agent in self.agents:
            print(f"Resetting agent {agent.agent_id}: type = {type(agent)}")
            agent.reset()  # Let agents handle their own reset logic
    ...
    def step(
        ...
        self.current_timestep += 1
        self.current_wholesale_price = self.wholesale_prices[self.current_timestep] if self.current_timestep < len(self.wholesale_prices) else self.wholesale_prices[-1]
        ...
        self.profit_history.append(rewards)
        
        observation = self._get_obs()
        info = {"timestep": self.current_timestep}
        return observation, rewards, self._check_termination(), self._check_truncation(), info
    ...
    def plot_results(self):
        ...
        plt.figure(figsize=(12, 5))
        plt.subplot(1, 2, 1)
        plt.plot(timesteps, self.wholesale_prices_history, marker="o", linestyle="-", color="orange")
        ...
        plt.subplot(1, 2, 2)
        plt.plot(timesteps, self.traded_quantities, marker="o", linestyle="-", color="purple")
    ...
    def plot_trading_pattern(self):
        """Plot trading patterns for wholesale market"""
        timesteps = range(1, self.current_timestep + 1)
        
        plt.figure(figsize=(12, 8))
        
        # Plot 1: Agent net demands over time
        plt.subplot(2, 2, 1)
        net_demand_matrix = np.array(self.net_demand_history)
        for i in range(self.n_agents):
            plt.plot(timesteps, net_demand_matrix[:, i], label=f"Agent {i}", alpha=0.7)
        ...
        # Plot 2: Wholesale price vs total demand
        plt.subplot(2, 2, 2)
        total_demand = np.sum(np.maximum(net_demand_matrix, 0), axis=1)
        plt.scatter(total_demand, self.wholesale_prices_history, alpha=0.6, color="red")
        plt.title("Wholesale Price vs Total Demand")
        plt.xlabel("Total Demand (MWh)")
        plt.ylabel("Wholesale Price (€/MWh)")
        plt.grid(True)
        
        # Plot 3: Daily pattern (if we have enough data)
        if len(timesteps) >= 24:
            plt.subplot(2, 2, 3)
            hours = np.array(timesteps) % 24
            plt.scatter(hours, self.wholesale_prices_history, alpha=0.6, color="green")
            plt.title("Wholesale Price by Hour of Day")
            plt.xlabel("Hour of Day")
            plt.ylabel("Wholesale Price (€/MWh)")
            plt.grid(True)
        ...
        plt.subplot(2, 2, 4)
        cumulative_volume = np.cumsum(self.traded_quantities)
        plt.plot(timesteps, cumulative_volume, marker="o", linestyle="-", color="blue")
