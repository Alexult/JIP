...
CONSUMER_RESPONSE_STRENGTH = (
    0.6  # Pure consumers are less aggressive at consumption redcution
)
PROSUMER_RESPONSE_STRENGTH = (
    0.8  # Prosumers with generation can curtail more aggressively
)
    ...
    for hour in range(24):
        ...
        for agent in agents:
            agent.calculate_net_demand(
                timestep
            )  # ← This calculates realistic flexible load!
...
def run_baseline_for_day(
    day_df: pd.DataFrame, agents: list, day_index: int = 0
) -> dict:
    ...
    for h in range(len(price)):
        ...
        for agent_data in hour_agent_data:
            ...
            initial_net_demand = (
                agent_data["fixed_load"] + realistic_flexible_load - baseline_generation
            )
            ...
            if initial_net_demand > 0:
                ...
                if p <= p_lo:
                    ...
                    r_consumption = (
                        response_strength * (p - p_lo) / max(1e-9, (p_hi - p_lo))
                    )
                ...
                if agent.agent_type == "prosumer":
                    ...
                    if p >= p_hi:
                        ...
                        r_consumption = (
                            PROSUMER_RESPONSE_STRENGTH * 1.2
                        )  # Even more aggressive
                        consumption_curtailed = min(
                            r_consumption * realistic_flexible_load,
                            realistic_flexible_load * 0.9,
                        )  # Max 90% reduction
                        generation_curtailed = (
                            0.0  # Don't curtail generation at high prices
                        )
                        ...
                        if baseline_generation > 0:
                            generation_curtailed = (
                                0.15 * baseline_generation
                            )  # Curtail 15% of generation
                    else:
            ...
            agent_net_demand = (
                agent_data["fixed_load"] + used_flexible - actual_generation
            )
            ...
            if agent.agent_type == "consumer":
                ...
                consumer_metrics["curtailed"] += max(
                    0, curtailed_flexible
                )  # Only positive curtailment
                consumer_metrics["used"] += used_flexible
            else:
                prosumer_metrics["cost"] += agent_cost
                prosumer_metrics["curtailed"] += max(
                    0, curtailed_flexible
                )  # Only positive curtailment
    ...
    return {
        ...
        "curtailment_share_%": 100.0
        * max(0, total_flex_curtailed)
        / max(1e-9, (total_fixed + total_flex_used + max(0, total_flex_curtailed))),
        "hi_threshold": p_hi,
        "lo_threshold": p_lo,
        # Enhanced metrics
...
def plot_day_comparison(days: dict, agents: list):
    ...
    for day in day_names:
        ...
        ax1.plot(
            range(24), price, label=f"{day}", linewidth=2, marker="o", markersize=4
        )
    ...
    for day in day_names:
        ...
        for hour_data in hourly_data["agents"]:
            hour_total_flexible = sum(
                [agent["flexible_load_used"] for agent in hour_data]
            )
            flexible_loads.append(hour_total_flexible)
        ax2.plot(
            range(24),
            flexible_loads,
            label=f"{day} Flexible Load",
            linewidth=2,
            marker="s",
        )
    ...
    for day in day_names:
        ...
        for hour_data in hourly_data["agents"]:
            hour_demand = sum(
                [
                    agent["fixed_load"] + agent["flexible_load_used"]
                    for agent in hour_data
                ]
            )
        ...
        ax3.plot(
            range(24), total_demands, label=f"{day} Demand", linewidth=2, linestyle="-"
        )
        ax3.plot(
            range(24),
            total_generations,
            label=f"{day} Generation",
            linewidth=2,
            linestyle="--",
        )
    ...
    ax5.bar(
        x - 0.25, total_demand, 0.25, label="Total Demand", color="#d62728", alpha=0.8
    )
    ...
    ax6 = axes[1, 2]
    consumer_curtailment = [
        all_metrics[day]["consumer_curtailment_MWh"] for day in day_names
    ]
    prosumer_curtailment = [
        all_metrics[day]["prosumer_curtailment_MWh"] for day in day_names
    ]
    ...
    plt.subplots_adjust(
        top=0.93, bottom=0.08, left=0.06, right=0.95, hspace=0.35, wspace=0.25
    )
    plt.show()
    ...
    print(
        f"{'Day':<12} {'Total Cost':<11} {'Consumer Cost':<13} {'Prosumer Cost':<13} {'Generation':<11} {'Net Demand':<11}"
    )
    ...
    for day in day_names:
        m = all_metrics[day]
        print(
            f"{day:<12} {m['total_cost_eur']:<11.2f} {m['consumer_cost_eur']:<13.2f} "
...
def main():
    ...
    for ag in agents:
        print(
            f"Agent {ag.agent_id}: {ag.agent_type}, Fixed: {ag.fixed_load} MWh, "
    ...
    for day_index, (day, df) in enumerate(days.items()):
        ...
        print(
            f"  Consumer cost      : € {metrics['consumer_cost_eur']:.2f} ({metrics['consumer_count']} agents)"
        )
        print(
            f"  Prosumer cost      : € {metrics['prosumer_cost_eur']:.2f} ({metrics['prosumer_count']} agents)"
        )
